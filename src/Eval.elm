...
apply func args =
    case Dict.get func primitives of
        Just f -> f args
        Nothing -> Atom "No such function"


primitives :  Dict String (List LispVal -> LispVal)
primitives = Dict.fromList [("+", numericBinop (+)),
              ("-", numericBinop (-)),
              ("*", numericBinop (*)),
              ("/", numericBinop (//))
...
unpackNum val =
    case val of
        Integer n -> n
        String n -> String.toInt n |> Maybe.withDefault 0
        List data -> List.head data |> Maybe.map unpackNum |> Maybe.withDefault 0
        _ -> 0
